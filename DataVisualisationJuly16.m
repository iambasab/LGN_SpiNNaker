...
tcrpop=10*scale_fact;
inpop=2*scale_fact;
trnpop=8*scale_fact;


TotalDuration=10000; % Total duration of simulation
TimeInt=0.001; %%Samplint time is 1 millisecond
...
locut=6000; 
hicut=7000; %% 1.5 sec is exactly 1 second after (before) onset of retinal inputs to TCR (IN) cells
% locut=3000; 
% hicut=4000;%% 6.5 sec is exactly 1 second before application of the second stint of retinal inputs to IN

% current_neuronpop=tcrpop;
current_neuronpop=inpop;    
% current_neuronpop=trnpop;
display ('Simulation 11 Hz')
for udn=2:10
%     udn=10
   
    
%     filename=sprintf('./SysManCyberB_June16/Sim2/TCRmempot_%d.dat',udn);
        filename=sprintf('./SysManCyberB_June16/Sim2/INmempot_%d.dat',udn);
%         filename=sprintf('./SysManCyberB_June16/Sim5/TRNmempot_%d.dat',udn);
    ...
    startind=1;
    mempot=zeros(current_neuronpop,TotalDuration);
    for i = 1:current_neuronpop
        try
            mempot(i,:) = neuron_par{1,3}(startind:TotalDuration+startind-1);
            %         mempot(i,:) = neuron_par{1,1}(startind:TotalDuration+startind-1);
            startind=TotalDuration+startind
        catch
            display('fault at i='),i,display('@'),udn
...
% figure,plot(100:TotalDuration-100,M(:,100:TotalDuration-100))
...
% lo_cutoff = 800;
% hi_cutoff = 9800;
% t=0:TimeInt:TotalDuration-1;
figure, plot(locut:hicut,M(:,locut:hicut))
hold on, plot(locut:hicut,meanM(1,locut:hicut),'r','linewidth',3)
...
%%ALSO FOR THE WHOLE MATRIX.
    ...
    hi_cutoff = hicut;
    for ms=1:size(M,1)
        filtData(ms,:) = filtfilt(B,A,M(ms,lo_cutoff:hi_cutoff));
    ...
    meanP=mean(Pmat,1);
    fr=hpsd.Frequencies;
    figure, plot(fr(1:400),Pmat(:,1:400))
    hold on, plot(fr(1:400),meanP(1:400),'r','linewidth',3)
    xlim([-0.5 101])
    set(gca,'XTick',[1 20 40 80 100],...
        'XTickLabel','|20|40|80|100', 'Fontsize',12)
    ...
    figure,plot(lo_cutoff:hi_cutoff,filtData(:,1:end))
    hold on,plot(lo_cutoff:hi_cutoff,mean_filtdata,'r','linewidth',3)

    % STFT
         x=Fc1; y=Fc2;
        [fr vismat]=fun_stft(meanM(1,locut:hicut), x, y);
        figure, imagesc([],fr((4*x+1):(4*y+1)),vismat((4*x+1):(4*y+1),2:end));
        xlabel('Time windows','Fontsize',14);
        ylabel('frequency(Hz)','Fontsize',14);
        axis('xy')
